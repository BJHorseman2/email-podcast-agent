...
AOL Email → AI Processing → ElevenLabs Voice → Email Delivery
"""
...
class PodcastAutomationAgent:
    def __init__(self, config_file='aol_complete_config.json'):
        """Initialize the complete automation agent"""
    ...
    def connect_to_aol(self):
        ...
        try:
            logging.info("🔗 Connecting to AOL email...")
            ...
            logging.error(f"❌ AOL connection failed: {e}")
            return None
    ...
    def find_target_email(self, imap):
        ...
        try:
            ...
            logging.info(f"🔍 Searching emails with: {search_criteria}")
            
            _, message_numbers = imap.search(None, search_criteria)
            
            if not message_numbers[0]:
                logging.info("📭 No new emails found matching criteria")
                return None
            ...
            # Check if subject matches our criteria
    ...
    def generate_podcast_script(self, email_data):
        """Use AI to generate engaging podcast script"""
        try:
            logging.info("🤖 Generating podcast script with AI...")
            ...
            Email Content:
            {email_data['body'][:3000]}  # Limit content length
            ...
            estimated_duration = round(word_count / 150)  # 150 words per minute
            
            logging.info(f"✅ Generated script: {word_count} words, ~{estimated_duration} min")
    ...
    def generate_voice_audio(self, script_data):
        ...
        try:
            ...
            if self.config['voice_generation']['api_key'] == "YOUR_ELEVENLABS_API_KEY":
                logging.warning("⚠️ ElevenLabs API key not configured, skipping voice generation")
                return None
    ...
    def send_podcast_email(self, email_data, script_data, audio_data):
        """Email the podcast to the user"""
        try:
            logging.info("📧 Sending podcast email...")
            ...
            logging.info(f"✅ Podcast emailed to {self.config['email_delivery']['recipient_email']}")
            return True
    ...
    def cleanup_old_files(self):
        ...
        try:
            cutoff_days = self.config['output']['cleanup_after_days']
            ...
            for filename in os.listdir(self.podcasts_dir):
                ...
                if os.path.isfile(filepath):
                    ...
                    if file_time < cutoff_time:
                        os.remove(filepath)
                        logging.info(f"🗑️ Cleaned up: {filename}")
        except Exception as e:
    ...
    def run_complete_automation(self):
        ...
        try:
            logging.info("🚀 Starting complete email-to-podcast automation...")
            
            # Step 1: Connect to AOL
            ...
            # Step 5: Email the podcast
            email_sent = self.send_podcast_email(email_data, script_data, audio_data)
            ...
            if email_sent:
                ...
                logging.info("🎉 Complete automation successful!")
    ...
    def test_all_components(self):
        ...
        # Test 1: AOL Connection
        print("1. Testing AOL email connection...")
        imap = self.connect_to_aol()
        if imap:
        ...
        if self.config['ai_processing']['api_key'] != "YOUR_OPENAI_API_KEY":
            ...
            print("2. OpenAI API not configured (using basic script generation)")
        
        # Test 3: ElevenLabs (if configured)
        if self.config['voice_generation']['api_key'] != "YOUR_ELEVENLABS_API_KEY":
            print("3. Testing ElevenLabs API...")
            try:
                test_script = {"script": "This is a test of the voice generation system."}
        ...
        print("4. Testing email configuration...")
        if (self.config['email_delivery']['sender_email'] != "YOUR_GMAIL@gmail.com" and
            self.config['email_delivery']['sender_password'] != "YOUR_GMAIL_APP_PASSWORD"):
            print("   ✅ Email settings configured")
        else:
            print("   ⚠️ Email delivery not configured")
    ...
    def schedule_automation(self):
        ...
        logging.info(f"📅 Scheduled automation for {run_time} daily")
        logging.info("🔄 Agent running in background...")
...
def main():
    """Main execution"""
    print("🎙️ Complete Email-to-Podcast Automation System")
    print("=" * 60)
    
    agent = PodcastAutomationAgent()
    
    # Test system
    if not agent.test_all_components():
    ...
    if success:
        ...
        print("   python3 complete_automation.py --schedule")
    else:
        print("\n⚠️ No emails processed (this is normal if no new emails)")
